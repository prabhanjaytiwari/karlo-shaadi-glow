  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: cityInfo } = await supabase
        .from("cities")
        .select("*")
        .ilike("name", city?.replace(/-/g, " ") || "")
        .single();

      if (cityInfo) {
        setCityData(cityInfo);

        // Load vendors for this city
        let query = supabase
          .from("vendors")
          .select(`
            *,
            cities (name, state),
            vendor_portfolio (image_url)
          `)
        ...
        setRelatedCategories(uniqueCategories);
      }

      // Load all cities for navigation
      ...
      <section className="pt-24 pb-12 bg-gradient-to-b from-rose-50/60 to-white">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <Badge className="bg-accent/15 text-accent border border-accent/30 mb-4">
            ...
            <h1 className="font-display font-bold text-4xl md:text-5xl mb-4">
              {category && category !== "all" ? (
                <>Best <span className="text-primary">{CATEGORY_NAMES[category]}</span> in {formatCityName(city || "")}</>
              ) : (
                <>Wedding Vendors in <span className="text-primary">{formatCityName(city || "")}</span></>
              )}
            </h1>
            ...
            <p className="text-lg text-muted-foreground mb-8">
              Discover {vendors.length}+ verified vendors • Compare prices • Read real reviews
            </p>

            {/* Category Filter Pills */}
            <div className="flex flex-wrap justify-center gap-2 mb-6">
              <Link to={`/vendors/${city?.toLowerCase()}`}>
                <Badge 
              ...
              {relatedCategories.slice(0, 8).map((cat) => (
                <Link key={cat} to={`/vendors/${city?.toLowerCase()}/${cat}`}>
                  <Badge 
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-center gap-8 md:gap-16">
            <div className="text-center">
              <p className="font-display font-bold text-2xl text-primary">{vendors.length}+</p>
              <p className="text-sm text-muted-foreground">Verified Vendors</p>
            </div>
            <div className="text-center">
              <p className="font-display font-bold text-2xl text-primary">4.8</p>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="font-display font-bold text-2xl text-center mb-8">
            Browse Vendors in Other Cities
          </h2>
          <div className="flex flex-wrap justify-center gap-3">
            {allCities.filter(c => c.name.toLowerCase() !== city?.replace(/-/g, " ").toLowerCase()).slice(0, 12).map((otherCity) => (
              <Link 
                key={otherCity.id} 
                to={`/vendors/${otherCity.name.toLowerCase().replace(/\s+/g, "-")}`}
              >
                <Badge variant="outline" className="cursor-pointer hover:bg-primary/10 py-2 px-4">
                  <MapPin className="h-3 w-3 mr-1" />
                  {otherCity.name}
                </Badge>
              </Link>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto prose prose-sm">
            <h2>
              {category && category !== "all" 
                ? `Why Choose Karlo Shaadi for ${CATEGORY_NAMES[category]} in ${formatCityName(city || "")}?`
                : `Find Your Perfect Wedding Vendors in ${formatCityName(city || "")}`
              }
            </h2>
            <p>
              Planning a wedding in {formatCityName(city || "")}? Karlo Shaadi connects you with the best 
              {category ? ` ${CATEGORY_NAMES[category]?.toLowerCase()}` : " wedding vendors"} in your city. 
              Every vendor on our platform is verified, ensuring you get quality service for your special day.
            </p>
            ...
            {category && CATEGORY_NAMES[category] && (
              <>
                <h3>Top {CATEGORY_NAMES[category]} Services in {formatCityName(city || "")}</h3>
                <p>
                  Our {CATEGORY_NAMES[category]?.toLowerCase()} in {formatCityName(city || "")} offer a wide range of 
                  services to match your wedding style and budget. From intimate ceremonies to grand celebrations, 
