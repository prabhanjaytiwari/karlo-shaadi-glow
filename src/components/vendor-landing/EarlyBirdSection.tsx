...
export function EarlyBirdSection() {
  ...
  return (
    <section className="py-16 md:py-28 px-4 sm:px-6 border-t-[3px] border-b-[3px] border-accent relative overflow-hidden"
      ...
      <div className="max-w-[1100px] mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="rounded-2xl px-6 sm:px-10 py-7 sm:py-9 flex flex-col sm:flex-row items-center justify-between gap-6 mb-10"
          style={{ background: "linear-gradient(90deg, hsl(350 72% 22%), hsl(350 72% 30%))", boxShadow: "0 20px 60px rgba(107,26,26,0.3)" }}>
          <div className="text-center sm:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold tracking-wider uppercase mb-3"
            ...
            <h2 className="font-display font-bold text-white leading-tight text-xl sm:text-3xl md:text-4xl">
              Early Vendor Window<br />
              <span style={{ color: "#E8B94A" }}>Closing Soon</span>
            </h2>
            <p className="text-white/65 mt-2 text-sm sm:text-base">First 100 vendors per city get lifetime perks</p>
          </div>
          <div className="flex gap-2.5 flex-shrink-0">
            {[
              ...
              <div key={i} className="text-center rounded-lg px-3.5 sm:px-4.5 py-3 min-w-[56px] sm:min-w-[64px]"
                style={{ background: "rgba(201,150,42,0.15)", border: "1px solid rgba(201,150,42,0.35)" }}>
                <div className="font-display font-bold leading-none text-2xl sm:text-4xl" style={{ color: "#E8B94A" }}>{u.v}</div>
                <div className="text-xs text-white/50 tracking-widest uppercase mt-1">{u.l}</div>
              </div>
            ))}
          </div>
        </motion.div>

        <p className="text-sm font-semibold tracking-[0.15em] uppercase mb-3 text-accent">Founding Member Benefits</p>
        <h2 className="font-display font-bold leading-tight mb-8 text-foreground text-2xl md:text-4xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {perks.map((p, i) => (
            <motion.div key={i}
              ...
              className="bg-white rounded-xl p-5 flex gap-4 items-start border border-accent/20 shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:border-accent/40 transition-all duration-200">
              <div className="text-2xl flex-shrink-0 mt-0.5">{p.icon}</div>
              <div>
                <div className="font-bold text-base mb-1 text-foreground">{p.title}</div>
                <div className="text-sm leading-relaxed text-muted-foreground">{p.desc}</div>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <button onClick={() => navigate("/vendor/onboarding")}
            className="px-8 py-4 rounded-xl font-bold text-base cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(201,150,42,0.4)]"
          ...
          <p className="text-sm text-muted-foreground mt-3">No credit card needed · 2 minute setup</p>
        </motion.div>
      </div>
    </section>
  );
